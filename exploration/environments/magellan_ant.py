...
def get_state_block(state):
    ...
    # See: https://github.com/nnaisense/MAX/issues/5#issue-807415360
    ...
    if x_block == 'low' and y_block == 'left':
        return 1
    elif x_block == 'mid' and y_block == 'left':
        return 2
    elif x_block == 'high' and y_block == 'left':
        return 3
    elif x_block == 'high' and y_block == 'center':
...
def rate_buffer(state_buffer):
    visited_blocks = [get_state_block(state) for state in state_buffer]
    unique_blocks = set(visited_blocks)
    n_unique = max(unique_blocks)
    return n_unique, unique_blocks
